...
module jtcop_bac06 #(
    parameter MASTER=0,     // One BAC06 chip will be the timing master
              REGION8_IS_16=0, // 1 = 8x8 mode picks from gfx_layout 16x16x4 (ActFancer PF0)
              SIMFILE="ba0.bin"
) (
    input             rst,
...
wire [15:0] cpu_ram;
wire [ 1:0] ncgsel;
reg  [ 7:0] row_addr, col_addr;   // extended to 8 bit for m_wide=2 shape0 (256 cols)
// reg  [ 9:0] rowscr_data;
...
generate
    if( MASTER ) begin
        ...
        jtframe_vtimer #(
            ...
            .VCNT_END   ( 9'd271    ),
            .VS_START   ( 9'h106    ),
            .HS_START   ( 9'd287    ),  // CRT centering (era 9'h130=304):
            .HB_START   ( 9'd255    ),
            .HB_END     ( 9'd383    ),
            .HINIT      ( 9'd255    )
        )   u_vtimer(
...
reg [ 8:0] veff_row;
reg [11:0] hn;   // extended for m_wide=2 (4096 px scroll range)
reg [12:1] pre_ram;
reg [11:0] tile_id;
reg [ 3:0] tile_pal;
...
// hflip = TILE_FLIPX MAME: ON quando col-major (mode[0] bit1 = 0).
// Row-major (PF1 chars $03, PF0 boot $02) → hflip=1 → MSB-first decode.
// Col-major (PF0 dopo $E080 → $01) → hflip=0 → LSB-first decode (TILE_FLIPX).
wire hflip = msbrow_en;
...
// MAME decbac06.cpp ActFancer m_wide=2:
//   GEOM_4X1 16x16: 256 cols × 16 rows
//     tile_shape0_scan column-major: idx = (row & 0xf) + ((0xFF - col[7:0]) << 4)
//     row-major:                     idx = (col & 0xf) + ((row & 0xf) << 4) + ((col & 0x1f0) << 4)
//   GEOM_2X2 16x16: 128 cols × 32 rows
//     tile_shape1_scan: idx = (col & 0xf) + ((row & 0x1f) << 4) + ((col & 0xf0) << 5)
//   GEOM_1X4 16x16:  64 cols × 64 rows
//     tile_shape2_scan: idx = (col & 0xf) + ((row & 0x3f) << 4) + ((col & 0x70) << 6)
// 8x8:
//   GEOM_4X1: 128×32 → idx = (col & 0x1f) + ((row & 0x1f) << 5) + ((col & 0x60) << 5)
//   GEOM_2X2:  64×64 → idx = (col & 0x1f) + ((row & 0x1f) << 5) + ((row & 0x20) << 5) + ((col & 0x20) << 6)
//   GEOM_1X4:  32×128 → idx = (col & 0x1f) + ((row & 0x7f) << 5)
//
// msbrow_en = control_0[0] bit1 (row-major mode flag in jtcop's nomenclature).
//   In jtcop convention, msbrow_en=0 means "MAME column-major" (TILE_FLIPX implicit).
//   In jtcop convention, msbrow_en=1 means "MAME row-major".
always @* begin
    row_addr = 0;
    col_addr = 0;
    pre_ram  = 0;
    case( geometry )
        GEOM_4X1: begin
            if( tile16_en ) begin
                // shape0 16x16: 256 × 16
                row_addr[3:0] = veff[7:4];  // 16 rows
                col_addr[7:0] = hn[11:4];   // 256 cols (8-bit)
                pre_ram = msbrow_en ?
                    // row-major: (col[3:0]) + (row[3:0]<<4) + (col[7:4]<<8)
                    { col_addr[7:4], row_addr[3:0], col_addr[3:0] } :
                    // column-major MAME: (row[3:0]) + ((0xFF - col[7:0]) << 4)
                    { ~col_addr[7:0], row_addr[3:0] };
            end else begin
                // shape0 8x8: 128 × 32
                row_addr[4:0] = veff[7:3];  // 32 rows
                col_addr[6:0] = hn[9:3];    // 128 cols (7-bit)
                pre_ram = { col_addr[6:5], row_addr[4:0], col_addr[4:0] };
            end
        end
        GEOM_2X2: begin
            if( tile16_en ) begin
                // shape1 16x16: 128 × 32
                row_addr[4:0] = veff[8:4];
                col_addr[6:0] = hn[10:4];
                pre_ram = { col_addr[6:4], row_addr[4:0], col_addr[3:0] };
            end else begin
                // shape1 8x8: 64 × 64
                row_addr[5:0] = veff[8:3];
                col_addr[5:0] = hn[8:3];
                pre_ram = { col_addr[5], row_addr[5], row_addr[4:0], col_addr[4:0] };
            end
        end
        default: begin // GEOM_1X4
            if( tile16_en ) begin
                // shape2 16x16: 64 × 64
                row_addr[5:0] = veff[9:4];
                col_addr[6:0] = hn[9:4];
                pre_ram = { col_addr[6:4], row_addr[5:0], col_addr[3:0] };
            end else begin
                // shape2 8x8: 32 × 128
                row_addr[6:0] = veff[9:3];
                col_addr[4:0] = hn[7:3];
                pre_ram = { row_addr[6:0], col_addr[4:0] };
            end
        end
    endcase
end
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        draw <= 0;
        if( HSl && !HS ) begin
            // Extended hscr to full 12-bit per m_wide=2 (4096 px tilemap)
            hn       <= hscr[11:0];
            tilecnt  <= 0;
            ram_good <= 0;
            pre_cs   <= 1;
...
// draw_data viene shiftato di 1 ad ogni pixel (vedi righe più in basso):
//   hflip=1 -> draw_data<<1, leggi MSB di ogni byte -> bit position fissa 7
//   hflip=0 -> draw_data>>1, leggi LSB di ogni byte -> bit position fissa 0
// gfx_decode_debug estrae il pen scegliendo bit_idx in base a hflip.
gfx_decode_debug u_dec_dbg (
    .rom_data         (draw_data),
...
always @(posedge clk, posedge rst) begin
    if( rst ) begin
        ...
        if( draw ) begin
            ...
            else if( REGION8_IS_16 )
                // 8x8 cell picks top-left 8x8 of 16x16 tile (gfx_actfan[1]).
                // MAME byte plane offset = 16+y (y=0..7) → DDR byte = T*128 + 64 + y*4
                // → rom_addr (word16-idx) = T*32 + 16 + y
                rom_addr <= { 1'b0, tile_id[10:0], 2'b10, veff[2:0] };
            else
